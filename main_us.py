...
from src.data.loader_us import get_dataloader
from src.model.bmunet import USModule
...
def train_one_epoch(
    ...
    # Train
    model.train()
    for i, (us3, us4, us5, us6, us7, us8, label) in enumerate(train_loader):
        logist, _ = model(us3, us4, us5, us6, us7, us8)
        ...
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.us.num_epochs}] Training [{i}/{len(train_loader)}] Train Loss1: {loss_category:1.5f} "
        )
...
def val_one_epoch(
    ...
    # Eval
    model.eval()
    for i, (us3, us4, us5, us6, us7, us8, label) in enumerate(val_loader):
        logist, _ = model(us3, us4, us5, us6, us7, us8)
        ...
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.us.num_epochs}] Validation [{i}/{len(val_loader)}] Val Loss: {loss:1.5f}"
        )
        step += 1
...
if __name__ == "__main__":
    ...
    utils.same_seeds(42)
    logging_dir = (
        os.path.join(os.getcwd(), 'logs', 'us', str(
    ...
    scheduler = LinearWarmupCosineAnnealingLR(
        ...
        max_epochs=config.us.num_epochs,
    )
    ...
    if config.trainer.resume:
        starting_epoch, step, val_step = utils.resume_train_state(
            config.us.save_dir, train_loader, val_loader, accelerator
        )

    # Start training
    accelerator.print("Start training！")

    for epoch in range(starting_epoch, config.us.num_epochs):
        # Train one epoch
        ...
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.us.num_epochs}] lr = {scheduler.get_last_lr()},  acc = {acc} %"
        )
        ...
        if acc > metric_saver.best_acc:
            ...
            accelerator.save_state(
                output_dir=os.path.join(os.getcwd(), config.us.save_dir, 'best')
            )

        accelerator.save_state(
            output_dir=os.path.join(os.getcwd(), config.us.save_dir, f"epoch_{epoch}")
        )
