...
from src.model.bmunet import MGModule, mg_load_pretrained_model
...
def train_one_epoch(
    ...
    # Train
    model.train()
    for i, (mg1, mg2, label) in enumerate(train_loader):
        with accelerator.accumulate(model):
            logist, _, _, _ = model(mg1, mg2)
            ...
            step += 1
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.mg.num_epochs}] Training [{i}/{len(train_loader)}] Train Loss: {loss_category:1.5f}"
        )
...
def val_one_epoch(
    ...
    # Eval
    model.eval()
    for i, (mg1, mg2, label) in enumerate(val_loader):
        logist, _, _, _ = model(mg1, mg2)
        ...
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.mg.num_epochs}] Validation [{i}/{len(val_loader)}] Val Loss: {loss:1.5f}"
        )
        step += 1
...
if __name__ == "__main__":
    ...
    utils.same_seeds(42)
    logging_dir = (
        os.path.join(os.getcwd(), 'logs', 'mg', str(
    ...
    # Load mirai pretrained weight
    model = mg_load_pretrained_model(model, config.mg.pre_trained_model)

    # Load dataset
    accelerator.print("Loading dataset...")
    train_loader, val_loader, stats_data_loader = get_dataloader(config)
    ...
    scheduler = LinearWarmupCosineAnnealingLR(
        ...
        max_epochs=config.mg.num_epochs,
    )
    ...
    if config.trainer.resume:
        starting_epoch, step, val_step = utils.resume_train_state(
            config.mg.save_dir, train_loader, val_loader, accelerator
        )

    # Start training
    accelerator.print("Start training！")

    for epoch in range(starting_epoch, config.mg.num_epochs):
        # Train one epoch
        ...
        accelerator.print(
            f"Epoch [{epoch + 1}/{config.mg.num_epochs}] lr = {scheduler.get_last_lr()},  acc = {acc} %"
        )
        ...
        if acc > metric_saver.best_acc:
            ...
            accelerator.save_state(
                output_dir=os.path.join(os.getcwd(), config.mg.save_dir, 'best')
            )

        accelerator.save_state(
            output_dir=os.path.join(os.getcwd(), config.mg.save_dir, f"epoch_{epoch}")
        )
